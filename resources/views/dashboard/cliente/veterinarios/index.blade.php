<x-app-layout>
    ...
    <div id="asignarModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <form id="asignarForm">
                    @csrf
                    <input type="hidden" name="veterinario_id" id="veterinario_id" value="">
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <form id="citaForm">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="cita_mascota_id" class="block text-sm font-medium text-gray-700">Seleccionar Mascota</label>
                            <select name="pet_id" id="cita_mascota_id" required
                        ...
                        <div>
                            ...
                            <select name="record_type" id="tipo_cita" required
                                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                                <option value="">Seleccionar tipo de cita</option>
                                <option value="consulta">Consulta General</option>
                                <option value="vacuna">Vacunación</option>
                                <option value="checkeo">Chequeo Rutinario</option>
                                <option value="emergencia">Emergencia</option>
                                <option value="operacion">Cirugía</option>
                            </select>
                        </div>
                        
                        <div>
                            <label for="fecha_solicitada" class="block text-sm font-medium text-gray-700">Fecha y Hora Solicitada</label>
                            <input type="datetime-local" name="requested_datetime" id="fecha_solicitada" required
                                   min="{{ now()->format('Y-m-d\TH:i') }}"
                                   class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        
                        <div>
                            <label for="observaciones_cita" class="block text-sm font-medium text-gray-700">Observaciones (Opcional)</label>
                            <textarea name="observations" id="observaciones_cita" rows="3"
                                      class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
    ...
    <script>
        ...
        document.getElementById('citaForm').addEventListener('submit', function(e) {
            ...
            fetch('{{ route("client.appointments.store") }}', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
