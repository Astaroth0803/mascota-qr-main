...
class PetController extends Controller
{
    ...
    public function dashboardCliente()
    {
        ...
        return view('client.index', compact('pets', 'stats', 'notifications', 'upcomingAppointments'));
    }
    ...
    public function adminDashboard()
    {
        ...
        return view('admin.index', compact('pets', 'stats', 'securityAlerts'));
    }
    ...
    public function clientNotifications()
    {
        ...
        // Obtener notificaciones de citas con paginación
        $notifications = \App\Models\AppointmentNotification::forUser($userId)
            ->with(['appointmentRequest.pet', 'sender'])
            ->orderBy('created_at', 'desc')
            ->paginate(20);
            
        $unreadCount = \App\Models\AppointmentNotification::forUser($userId)
            ->unread()
            ->count();
    ...
    public function show(Pet $pet)
    {
        ...
        return view('pets.show', ['pet' => $cachedPet]);
    }
    ...
    {
        try {
            // Verificar que el usuario tenga permiso para ver esta mascota
            if (!Auth::user()->can('ver_mascotas') ||
                (Auth::user()->id !== $pet->user_id && Auth::user()->email !== $pet->correo_owner)) {
                return redirect()->route('dashboard.cliente')
            ...
            return view('pets.vaccination-history', compact('pet', 'recordTypes'));
            
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Error en showVaccinationHistory: ' . $e->getMessage(), [
            ...
            return redirect()->route('dashboard.cliente')
                ->with('error', 'Error al cargar el historial médico: ' . $e->getMessage());
        }
    }
    ...
    public function debugRecords(Pet $pet)
    {
        ...
        return view('pets.debug-records', compact('pet', 'sqlRecords'));
    }
    ...
    public function vaccinationHistory(Pet $pet)
    {
        ...
        return view('pets.vaccination-history', compact('pet', 'vaccinationRecords'));
    }
}
