...
@php
    ...
    $subtitle = 'Información completa de tu mascota';
@endphp
...
<div class="w-full">
    ...
    <nav class="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 mb-6">
        <a href="{{ route('dashboard.cliente.index') }}" class="hover:text-gray-700 dark:hover:text-gray-300 transition-colors">Dashboard</a>
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
        </svg>
        <span class="text-gray-900 dark:text-white font-medium">Detalles de Mascota</span>
    </nav>
    
    <!-- Título Principal -->
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">{{ $pet->nombre }}</h1>
            <p class="text-gray-600 dark:text-gray-300 mt-1">Información completa de tu mascota</p>
        </div>
    ...
    <div class="w-full">
        <div class="px-2 sm:px-4 lg:px-6 xl:px-8 py-4 lg:py-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6 relative overflow-hidden">
                <!-- QR Code Badge -->
                @if($pet->qr_code)
                ...
                <div class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
                    ...
                    <div class="relative group">
                        <div class="w-20 h-20 md:w-24 md:h-24 bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 rounded-2xl shadow-lg flex items-center justify-center overflow-hidden">
                            @if($pet->profile_image)
                                <img src="{{ Storage::url($pet->profile_image) }}" alt="{{ $pet->nombre }}" class="w-full h-full object-cover rounded-2xl">
                            @else
                                <svg class="w-10 h-10 md:w-12 md:h-12 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="text-center md:text-left flex-1">
                        <div class="mb-3">
                            <p class="text-lg text-gray-700 dark:text-gray-300 font-medium">{{ $pet->especie }} • {{ $pet->raza }}</p>
                        </div>
                        
                        <!-- Información rápida compacta -->
                        <div class="flex flex-wrap justify-center md:justify-start gap-3">
                            <div class="flex items-center bg-gray-50 dark:bg-gray-700 rounded-xl px-3 py-2 border border-gray-200 dark:border-gray-600">
                                ...
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $pet->edad_anios }}a {{ $pet->edad_meses }}m</span>
                            </div>
                            <div class="flex items-center bg-gray-50 dark:bg-gray-700 rounded-xl px-3 py-2 border border-gray-200 dark:border-gray-600">
                                <svg class="w-4 h-4 mr-2 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                                </svg>
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $pet->sexo }}</span>
                            </div>
                            @if($pet->peso)
                                <div class="flex items-center bg-gray-50 dark:bg-gray-700 rounded-xl px-3 py-2 border border-gray-200 dark:border-gray-600">
                                    <svg class="w-4 h-4 mr-2 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $pet->peso }}kg</span>
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center p-4 bg-gray-50 dark:bg-gray-700 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                            ...
                            <div class="flex-1">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Raza</p>
                                <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $pet->raza }}</p>
                            </div>
                        </div>
