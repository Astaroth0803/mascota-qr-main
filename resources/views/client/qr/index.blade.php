...
@section('title', 'Gestor de Códigos QR')

@php
    $title = 'Gestor de Códigos QR';
    $subtitle = 'Gestiona los códigos QR de tus mascotas';
@endphp
...
<div class="w-full">
    <!-- Header Principal -->
    <div class="mb-6">
        <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">Gestor de Códigos QR</h1>
        <p class="text-gray-600 mt-1">Gestiona los códigos QR de tus mascotas</p>
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6 mb-6 lg:mb-8">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Total Mascotas</p>
                        <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['total_pets'] }}</p>
                    </div>
                </div>
            </div>
    ...
    <div id="qrModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
        <div class="relative top-20 mx-auto p-6 border w-11/12 md:w-1/2 lg:w-1/3 shadow-lg rounded-lg bg-white">
            <div class="mt-3">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-semibold text-gray-900" id="qrModalTitle">Código QR</h3>
                ...
                <div class="text-center">
                    ...
                    <div class="bg-gray-50 rounded-xl p-4 mb-6">
                        <div class="space-y-3">
                            <div>
                                <p class="text-sm font-medium text-gray-500 mb-1">Código:</p>
                                <div class="bg-white px-3 py-2 rounded-lg border border-gray-200">
                                    <p class="text-sm font-mono text-gray-900" id="qrCodeText"></p>
                                </div>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-500 mb-1">URL:</p>
                                <div class="bg-white px-3 py-2 rounded-lg border border-gray-200">
                                    <p class="text-sm font-mono text-gray-900 break-all" id="qrUrlText"></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4 mb-6 border border-indigo-200">
                        <div class="flex items-center mb-3">
                            ...
                            <h4 class="text-lg font-semibold text-gray-800">Compartir Perfil de Mascota</h4>
                        </div>
                        ...
                        <button onclick="shareQR()" class="w-full inline-flex items-center justify-center px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-black text-sm font-medium rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 shadow-lg">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.367 2.684 3 3 0 00-5.367-2.684z"></path>
                            </svg>
                            Compartir Perfil
                ...
                <div class="flex justify-end">
                    <button onclick="closeQRModal()" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition-colors">
                        Cerrar
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        function showQRModal(petName, qrCode, qrUrl) {
            console.log('showQRModal called with:', { petName, qrCode, qrUrl });
        ...
        function shareQR() {
            if (navigator.share) {
                ...
                navigator.share({
                    title: `Perfil de Mascota - ${document.getElementById('qrModalTitle').textContent.replace('Código QR - ', '')}`,
                    text: `Mira el perfil de mi mascota: ${document.getElementById('qrModalTitle').textContent.replace('Código QR - ', '')}`,
                    url: currentQRUrl
                }).then(() => {
